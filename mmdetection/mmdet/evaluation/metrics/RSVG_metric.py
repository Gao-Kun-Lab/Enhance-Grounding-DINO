...
class RSVGMetric(BaseMetric):
        ...
        for data_sample in data_samples:
            ...
            if overlap.shape[0] == 0:
                ...
                return self.results.append((overlap.sum(), union.sum(), torch.tensor(0, device=label.device), gt_num,
                                            Pr2, Pr3, Pr4, Pr5, Pr6, Pr7, Pr8, Pr9, gt_num, TP, FP, FN, redundancy, bg))
            ...
            if iou.shape[0] > 0:
                iou = iou.sort(descending=True)[0]
                iou_max = iou.max()
            ...
            self.results.append((overlap.sum(), union.sum(), iou.sum(), gt_num, Pr2, Pr3, Pr4,
                                 Pr5, Pr6, Pr7, Pr8, Pr9, gt_num, TP, FP, FN, redundancy, bg))
    ...
    def compute_metrics(self, results: list) -> dict:
        ...
        cum_gt_total = sum(results[12])
        TP = sum(results[13])
        FP = sum(results[14])
        FN = sum(results[15])
        redundancy = sum(results[16])
        bg = sum(results[17])
        total = TP + FP + FN
        ...
        metrics['Pr_maxiou@0.2'] = cum_Pr2 * 100 / cum_gt_total
        # if 'Pr@0.3' in self.metrics:
        metrics['Pr_maxiou@0.3'] = cum_Pr3 * 100 / cum_gt_total
        # if 'Pr@0.4' in self.metrics:
        metrics['Pr_maxiou@0.4'] = cum_Pr4 * 100 / cum_gt_total
        if 'Pr@0.5' in self.metrics:
            metrics['Pr_maxiou@0.5'] = cum_Pr5 * 100 / cum_gt_total
        if 'Pr@0.6' in self.metrics:
            metrics['Pr_maxiou@0.6'] = cum_Pr6 * 100 / cum_gt_total
        if 'Pr@0.7' in self.metrics:
            metrics['Pr_maxiou@0.7'] = cum_Pr7 * 100 / cum_gt_total
        if 'Pr@0.8' in self.metrics:
            metrics['Pr_maxiou@0.8'] = cum_Pr8 * 100 / cum_gt_total
        if 'Pr@0.9' in self.metrics:
            metrics['Pr_maxiou@0.9'] = cum_Pr9 * 100 / cum_gt_total
