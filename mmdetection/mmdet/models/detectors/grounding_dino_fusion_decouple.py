...
class GroundingDINO_fusion_decouple(DINO):
    ...
    def predict(self, batch_inputs, batch_data_samples, rescale: bool = True):
        ...
        for data_sample, pred_instances, entity in zip(batch_data_samples,
            ...
            # bboxes, labels, scores = pred_instances.bboxes, pred_instances.labels, pred_instances.scores
            # label_names = pred_instances.label_names
            # # class_sentinel = ('Ships-moored-in-the-port',
            # #                  'High-speed-ship',
            # #                  'Ship-on-the-sea',
            # #                  'Ship-on-the-river',
            # #                  'white-ship',
            ...
            # #                  'black-ship',
            # #                  'small-ship',)
            # txt_name = label_names[0] + '.txt'
            # img_name = data_sample.img_path.split('/')[-1].split('.')[0]
            # with open(os.path.join('/data1/detection_data/datasets_Sentinel2/myself/imshow_result/', txt_name), 'a') as f:
            #     for bbox, label, score in zip(bboxes, labels, scores):
            #         out_str = img_name + ' ' + str(score.item()) + ' ' + ' '.join([str(_.item()) for _ in list(bbox)]) + '\n'
            #         f.write(out_str)
            # f.close()
