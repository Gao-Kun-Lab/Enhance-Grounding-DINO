...
from ..layers.transformer.grounding_dino_sparse_layers import (
    GroundingDinoNofusionTransformerDecoder, GroundingDinoNofusionTransformerEncoder
)
...
class GroundingDINO_no_fusion(DINO):
    ...
    def __init__(self, language_model, anchor_num=9, *args, **kwargs) -> None:
    ...
    def _init_layers(self) -> None:
        ...
        self.encoder = GroundingDinoNofusionTransformerEncoder(**self.encoder)
        self.decoder = GroundingDinoNofusionTransformerDecoder(**self.decoder)
        ...
        self.memory_multi_anchor = nn.Linear(self.embed_dims, self.embed_dims * self.anchor_num)
        # text modules
    ...
    def pre_decoder(
        ...
        bs, token_num, c = memory.shape

        output_memory, output_proposals = self.gen_encoder_output_proposals(
            memory, memory_mask, spatial_shapes)

        output_memory = self.memory_multi_anchor(output_memory)
        output_proposals = torch.stack(output_proposals).permute(1,2,0,3).reshape(bs, token_num * self.anchor_num, 4)
        output_memory = output_memory.reshape(bs, token_num, self.anchor_num, c).reshape(bs, token_num * self.anchor_num, c)
        ...
        ########### add memory feat ##########
        v_gather_query = output_memory.gather(1, topk_indices.unsqueeze(-1).repeat(1, 1, memory.shape[-1]))
        v_query = v_gather_query.detach()
        v_query = torch.tensor(v_query, requires_grad=True)
        query = query + v_query
