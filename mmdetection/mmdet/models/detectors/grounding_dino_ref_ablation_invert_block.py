...
from ..layers.transformer.grounding_dino_layers import (
    GroundingDinoTransformerDecoder, GroundingDinoTransformerEncoder, GroundingDinoAblationInvertBlockTransformerEncoder)
...
class GroundingDINO_ref_ablation_invert_block(DINO):
    ...
    def _init_layers(self) -> None:
        ...
        self.encoder = GroundingDinoAblationInvertBlockTransformerEncoder(**self.encoder)
    ...
    def predict(self, batch_inputs, batch_data_samples, rescale: bool = True):
        ...
        if self.show_result:
            output_dir = './work_dirs/sparse_imshow/ours_imshow_RSVG/'
            if not os.path.exists(output_dir):
                os.mkdir(output_dir)
            img_path = batch_data_samples[0].img_path
            img = cv2.imread(img_path)
            bbox = results_list[0]['bboxes'][0]
            score = results_list[0]['scores'][0]
            score = str(round(score.item(), 2))
            text = batch_data_samples[0].text
            font = cv2.FONT_HERSHEY_SIMPLEX
            position = (int(bbox[2].item()), int(bbox[1].item()))  # 文本在图像中的位置 (x, y)
            font_scale = 1  # 字体大小
            font_color = (0, 255, 0)  # 字体颜色 (B, G, R)
            line_type = 2  # 线条类型
            cv2.putText(img, score, position, font, font_scale, font_color, line_type)
            cv2.rectangle(img,
            ...
            cv2.imwrite(osp.join(output_dir, output_name),img)
        if self.show_feature:
            ...
            for i in range(bs):
                ...
                # plt.imshow(images)
                # plt.show()
                output_dir = './work_dirs/sparse_imshow/cam_RSVG_HR_ours'
