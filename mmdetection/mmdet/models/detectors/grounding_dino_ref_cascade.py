...
from ..layers.transformer.grounding_dino_layers import (
    GroundingDinoTransformerDecoder, GroundingDinoTransformerEncoder, GroundingDinoCascadeTransformerEncoder)
...
class GroundingDINO_ref_cascade(DINO):
    ...
    def __init__(self, language_model, show_feature=False, show_result=False, guide_loss=False, *args, **kwargs) -> None:
    ...
    def _init_layers(self) -> None:
        ...
        self.encoder = GroundingDinoCascadeTransformerEncoder(**self.encoder)
    ...
    def pre_decoder(
        ...
        if self.guide_loss:
            memory_inter = memory[:-1]
            memory_text_inter = memory_text[:-1]
            memory = memory[-1]
            memory_text = memory_text[-1]
            enc_outputs_class_inter_list = []
            for i in range(len(memory_inter)):
                enc_outputs_class_inter = self.bbox_head.cls_branches[
                    self.decoder.num_layers + i + 1](memory_inter[i], memory_text_inter[i],
                                             text_token_mask)
                enc_outputs_class_inter_list.append(enc_outputs_class_inter)
        bs, _, c = memory.shape
        ...
        if self.guide_loss and self.training:
            topk_score_inter_list = []
            for enc_outputs_class_inter in enc_outputs_class_inter_list:
                topk_score_inter = torch.gather(
                    enc_outputs_class_inter, 1,
                    topk_indices.unsqueeze(-1).repeat(1, 1, cls_out_features))
                topk_score_inter_list.append(topk_score_inter)
            head_inputs_dict['enc_outputs_inter_class'] = topk_score_inter_list
        ...
        if self.show_feature:
            ...
            for i in range(bs):
                ...
                for layer in range(len(self.activations)):
                    ...
                    # with open(osp.join(output_dir, 'gt_bg_trainval_delta_median_grounding_dino.txt'), 'a') as f:
                    #     f.write(out_str + '\n')
                # if show:
