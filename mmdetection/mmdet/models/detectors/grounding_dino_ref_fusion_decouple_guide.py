...
from ..layers.transformer.grounding_dino_layers import (
    GroundingDinoTransformerDecoder, GroundingDinoTransformerEncoder, GroundingDinoDecoupleGuideTransformerEncoder)
...
class GroundingDINO_ref_fusion_decouple_guide(DINO):
    ...
    def _init_layers(self) -> None:
        ...
        self.encoder = GroundingDinoDecoupleGuideTransformerEncoder(**self.encoder)
    ...
    def predict(self, batch_inputs, batch_data_samples, rescale: bool = True):
        ...
        for i, t in enumerate(text_prompts):
            if "plane" in t:
                # text_prompts[i] = "all planes in the image"
                show_result = True
        ...
        if show_result:
            output_dir = './work_dirs/RSVG_out_imshow/fusion_decouple_IoU_gt_imshow_RSVG_HR/'
            if not os.path.exists(output_dir):
                os.mkdir(output_dir)
            for i in range(len(batch_data_samples)):
                img_path = batch_data_samples[i].img_path
                gt_bbox = batch_data_samples[i].gt_bboxes.tensor[0]
                text = text_prompts[i]
                img_id = batch_data_samples[i].img_id
                img = cv2.imread(img_path)
                # bbox = results_list[i]['bboxes'][0]
                ...
                font_scale = 1  # 字体大小
                font_color = (255, 255, 255)  # 字体颜色 (B, G, R)
                line_type = 2  # 线条类型
                ...
                for ind, bbox in enumerate(bboxes):
                    cv2.rectangle(img,
                                  ...
                                  (0, 255, 0), 3)
                    position = (int(bbox[2].item()), int(bbox[1].item()))
                    cv2.putText(img, str(round(scores[ind].item(), 2)), position, font, font_scale, font_color, line_type)
                # cv2.rectangle(img,
                #               (int(gt_bbox[0].item()), int(gt_bbox[1].item())),
                #               (int(gt_bbox[2].item()),int(gt_bbox[3].item())),
                #               (0, 0, 255), 3)

                import random
                id = '_' + str(img_id) + '_' + str(random.randint(0, 1000)) + str(random.randint(0, 100))
                cv2.imwrite(osp.join(output_dir, text + id + '.jpg'),img)
                # plt.imshow(img[..., ::-1])
                # plt.show()
        if self.show_result:
            output_dir = './work_dirs/RSVG_out_imshow/fusion_decouple_IoU_gt_imshow_RSVG_HR/'
            ...
            for i in range(len(batch_data_samples)):
                ...
                # cv2.putText(img, text, position, font, font_scale, font_color, line_type)
                import random
                id = '_' + str(random.randint(0, 10000)) + '_' + str(random.randint(0, 1000)) + str(random.randint(0, 100))
                cv2.imwrite(osp.join(output_dir, text + id + '.jpg'),img)
                # plt.imshow(img[..., ::-1])
                # plt.show()
        if self.show_feature:
            ...
            for i in range(bs):
                ...
                for layer in range(len(self.activations)):
                    ...
                    for level, vf in enumerate(visual_feats):
                        ...
                        delta = gt_cam.median() - bg_cam.median()
                    ...
                    with open(osp.join(output_dir, 'gt_bg_trainval_delta_median_fusion_decouple.txt'), 'a') as f:
                        f.write(out_str + '\n')
                # if show:
                images = np.concatenate(images, axis=1)
                # plt.imshow(images)
                # plt.show()
                output_dir = 'WORK_DIR/mmdetection/work_dirs/RSVG_out_imshow/cam_RSVG_HR_show'
