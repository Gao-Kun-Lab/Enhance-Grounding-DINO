...
from ..layers.transformer.grounding_dino_layers import (
    GroundingDinoTransformerDecoder, GroundingDinoTransformerEncoder, GroundingDinoGuideTransformerEncoder)
...
class GroundingDINO_ref_guide(DINO):
    ...
    def _init_layers(self) -> None:
        ...
        self.encoder = GroundingDinoGuideTransformerEncoder(**self.encoder)
    ...
    def pre_decoder(
        ...
        #     output_ind = memory[:, start:start + feat_ind, :]
        #     if memory_mask is not None:
        #         memory_mask_.append(memory_mask[:, start:start + feat_ind])
        #     start += feat_ind
        #     output_.append(output_ind)
        #
        # memory_lite = torch.cat([output_[1], output_[-1]], dim=1)
        # if memory_mask is not None:
        #     memory_mask_lite = torch.cat([memory_mask_[1], memory_mask_[-1]], dim=1)
        # else:
        #     memory_mask_lite = None
        # spatial_shapes_lite = torch.stack((spatial_shapes[1], spatial_shapes[-1]), 0)
