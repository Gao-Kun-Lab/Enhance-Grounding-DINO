...
class GroundingDINO_ref_similar(DINO):
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        feature_image = self.activations[0][0]
        feature_text = self.activations[0][1]
        text_token_mask = head_inputs_dict['text_token_mask']

        cls_scores = self.bbox_head.cls_branches[-1](feature_image, feature_text,
                                     text_token_mask)
        ...
        for i in range(bs):
            ...
            positive_map = batch_data_samples[i].gt_instances.positive_maps
            ...
            label = cls_scores.new_full((cls_scores.shape[1], cls_scores.shape[-1]),
                                        0,
                                        dtype=torch.float32)
            label[gt_label==1] = positive_map
            label_weight = cls_scores.new_ones(cls_scores.shape[1])
            labels.append(label)
            label_weights.append(label_weight)
            cls_avg_factor += gt_label[gt_label==1].shape[0]
        labels = torch.stack(labels, dim=0)
        label_weights = torch.stack(label_weights, dim=0)
        assert (text_token_mask.dim() == 2)
        text_masks = text_token_mask.new_zeros(
            (text_token_mask.size(0), cls_scores.shape[-1]))
        ...
        losses['loss_fusion'] = loss_fusion
