...
class DetrTransformerWeightEncoderLayer(BaseModule):
    """Implements encoder layer in DETR transformer.
    ...
    def _init_layers(self) -> None:
        """Initialize self-attention, FFN, and normalization."""
        self.self_attn = MultiheadAttention_weight(**self.self_attn_cfg)
        self.embed_dims = self.self_attn.embed_dims
        self.ffn = FFN(**self.ffn_cfg)
    ...
    def forward(self, query: Tensor, P4:Tensor, P3: Tensor, spatial_shapes, query_pos: Tensor,
                key_padding_mask: Tensor, **kwargs) -> Tensor:
        """Forward function of an encoder layer.
        ...
        if self.before_FFN:
            P4 = P4 + F.interpolate(
                query.reshape(bs, H5, W5, dim).permute(0, -1, 1, 2),
        ...
        output = torch.cat([P3, P4, query], dim=1)
        return output
