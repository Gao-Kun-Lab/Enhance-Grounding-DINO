...
class HungarianAssigner_ref(BaseAssigner):
    ...
    def assign(self,
        ...
        if recored_txt:
            ...
            if self.record_txt:
                ...
                for i in enumerate(matched_iou_list):
                    size = aera[i]
                    if size < small_object:
                        out_str_small += str(round(matched_iou_list[i], 4)) + ' '
                    elif size > medium_object:
                        out_str_large += str(round(matched_iou_list[i], 4)) + ' '
                    else:
                        out_str_medium += str(round(matched_iou_list[i], 4)) + ' '
                out_str = out_str_small + out_str_medium + out_str_large
            ...
            if matched_row_inds.shape[0] != 0 and pred_instances.bboxes.shape[0] < 5000:
                ...
                if not osp.exists(out_dir):
                    os.mkdir(out_dir)
                scale_factor = gt_instances.bboxes.new_tensor(
                    img_meta['scale_factor']).repeat((1, 2))

                imshow_bbox8(img_meta['img_path'], img_meta['flip_direction'],
                             gt_instances.bboxes/scale_factor, bbox_preds_assign/scale_factor,
                             None,
                             # pred_instances.bboxes/scale_factor,
                             None, imshow, output, out_dir, img_meta['text'])
                # if self.record_txt:
                #     txt_out_dir = './work_dirs/record_txt/'
...
def imshow_bbox8(img_dir, flip_direction, gt_bbox, bbox_pred, bbox_preds_all=None, high_quality_neg=None, imshow=True, output=False, out_dir = None, text=None):
    if not imshow and not output:
        ...
        if P <= 0.0003:
            img = draw_line(img_dir, flip_direction, bbox_preds_all, gt_bbox, bbox_pred, high_quality_neg)
            img_name = img_dir.split('/')[-1].split('.')[0] + text + '.jpg'
            cv2.imwrite(osp.join(out_dir, img_name), img)
...
def draw_line(img_dir, flip_direction, bbox_preds_all, gt_bbox, bbox_pred, high_quality_neg):
    ...
    if bbox_preds_all is not None:
        for line in bbox_preds_all.tolist():
            ...
            cv2.line(img, points[0], points[1], color=(0, 0, 255), thickness=thickness_)
            cv2.line(img, points[1], points[2], color=(0, 0, 255), thickness=thickness_)
            cv2.line(img, points[2], points[3], color=(0, 0, 255), thickness=thickness_)
            cv2.line(img, points[3], points[0], color=(0, 0, 255), thickness=thickness_)
