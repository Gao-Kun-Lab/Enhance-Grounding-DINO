...
class BiMultiHeadAttention_debug(nn.Module):
    """Bidirectional fusion Multi-Head Attention layer.
    ...
    def __init__(self,
                 ...
                 num_heads: int,
                 dropout: float = 0.1):
        super(BiMultiHeadAttention_debug, self).__init__()
    ...
    def forward(
        ...
        return attn_output_v, attn_output_l, attn_weights.view(bsz, self.num_heads, tgt_len, src_len)

class BiAttentionBlock_debug(nn.Module):
    """BiAttentionBlock Module:
    ...
    def __init__(self,
        ...
        self.layer_norm_v = nn.LayerNorm(v_dim)
        self.layer_norm_l = nn.LayerNorm(l_dim)
        self.attn = BiMultiHeadAttention_debug(
            v_dim=v_dim,
            l_dim=l_dim,
            embed_dim=embed_dim,
            num_heads=num_heads,
    ...
    def single_attention_call(
        ...
        visual = self.layer_norm_v(visual)
        lang = self.layer_norm_l(lang)
        delta_v, delta_l, attn_weights = self.attn(
            visual,
            lang,
            attention_mask_v=attention_mask_v,
            attention_mask_l=attention_mask_l)
        ...
        visual = visual + self.drop_path(self.gamma_v * delta_v)
        lang = lang + self.drop_path(self.gamma_l * delta_l)
        return visual, lang, attn_weights


class SingleScaleBiAttentionBlock_debug(BiAttentionBlock_debug):
    """This is a single-scale implementation of `BiAttentionBlock`.
    ...
    def forward(self,
        ...
        Args:
            ...
            attention_mask_l (_type_, optional): Language feature attention
                mask.Defaults to None.
        """
        new_v, new_lang_feature, attn_weights = self.single_attention_call(
            visual_feature,
            lang_feature,
            attention_mask_v=attention_mask_v,
            attention_mask_l=attention_mask_l)
        return new_v, new_lang_feature, attn_weights
