...
class DetLocalVisualizer(Visualizer):
    ...
    def add_datasample(
        ...
        if show:
            try:
                captaion = data_sample.text
                cv2.putText(drawn_img, captaion, (100, 100), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 255, 255), 2)
        ...
        if out_file is not None:
            try:
                if show:
                    basename = osp.basename(out_file)
                    mmcv.imwrite(drawn_img[..., ::-1], out_file.replace(basename, basename.split('.')[0] + captaion + '.jpg'))
                else:
                    captaion = data_sample.text
                    cv2.putText(drawn_img, captaion, (100, 100), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 255, 255), 2)
                    basename = osp.basename(out_file)
